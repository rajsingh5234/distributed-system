...
describe('POST /auth/refresh', () => {
  describe('Given a valid refresh token', () => {
    ...
    it('should set new access token in cookie', async () => {
      ...
      const accessTokenCookie = responseCookies.find((cookie: string) =>
    ...
    it('should set new refresh token in cookie', async () => {
      ...
      const refreshTokenCookie = responseCookies.find((cookie: string) =>
    ...
    it('should rotate refresh token in db', async () => {
      ...
      const oldRefreshTokenCookie = registerCookies.find((c: string) =>
        c.startsWith('refreshToken=')
      );
      const oldRefreshToken = oldRefreshTokenCookie
        ?.split(';')[0]
        .split('=')[1];
      ...
      const newRefreshTokenCookie = responseCookies.find((c: string) =>
        c.startsWith('refreshToken=')
      );
      ...
      const oldTokenInDb = await refreshTokenRepository.findById(
        oldPayload.jwtid
      );
      const newTokenInDb = await refreshTokenRepository.findById(
        newPayload.jwtid
      );
